...
const CRYPTOAPIS_SERVER_INFO = {
    name: "cryptoapis-contracts",
    version: "0.1.0",
    title: "CryptoAPIs Contracts",
...
export async function startContractsServer(opts: StartOptions) {
    ...
    if (opts.transport === "stdio") {
        ...
        logger.logInfo("cryptoapis-contracts MCP running (stdio)");
        return;
    }
    ...
    app.listen(port, host, () => {
        logger.logInfo(`cryptoapis-contracts MCP running (http) at http://${host}:${port}${path}`);
